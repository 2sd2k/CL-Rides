...
export function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }) {
  ...
  useEffect(() => {
    ...
    setRailEnabled(hasDeckStage && !!document.querySelector('deck-stage')?._railEnabled);
    if (!hasDeckStage) return undefined;
    const onMsg = (e) => {
  ...
  useEffect(() => {
    try { setRailVisible(localStorage.getItem('deck-stage.railVisible') !== '0'); } catch {}
  }, []);
  const toggleRail = (on) => {
    setRailVisible(on);
